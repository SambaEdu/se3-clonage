	...
	function check_machine(num_parc,mode) {
		for(i=0;i<$max_eff_parc;i++){
			...
			if(!isset($validation_parametres)) {
				...
				echo "<td>";
				echo "<input type='text' name='disk' value='sda1' size='5' /><br />\n";
				...
				for($i=0;$i<count($tab_diskmodule);$i++){
					echo "<option value='$tab_diskmodule[$i]'>$tab_diskmodule[$i]</option>\n";
				}
				echo "</select>\n";
				echo "</td></tr>\n";

				echo "<tr><td>Pilote réseau: </td><td>\n";
				echo "<select name='netmodule'>\n";
				echo "<option value='AUTO' selected='true'>Auto-detect</option>\n";
				for($i=0;$i<count($tab_diskmodule);$i++){
					echo "<option value='$tab_netmodule[$i]'>$tab_netmodule[$i]</option>\n";
				}
				echo "</select>\n";
				echo "</td></tr>\n";
				...
				echo "<td>\n";
					...
					echo "<tr><td valign='top'><input type='radio' id='shutdown_reboot_wait2' name='shutdown_reboot' value='wait2' checked /> </td><td><label for='shutdown_reboot_wait2'>Redémarrer les machines sans session ouverte et attendre le reboot pour les machines qui ont des sessions ouvertes,</label></td></tr>\n";
					echo "<tr><td valign='top'><input type='radio' id='shutdown_reboot_reboot' name='shutdown_reboot' value='reboot' /> </td><td><label for='shutdown_reboot_reboot'>Redémarrer les machines même si une session est ouverte (<i>pô cool</i>).</label></td></tr>\n";
					echo "</table>\n";
				...
				echo "<td>\n";
				echo "<input type='text' name='min_receivers' value='1' size='3' />\n";
				echo "<br />\n";
				...
				*/
				...
				//$resultat=exec("/usr/bin/sudo $chemin/pxe_gen_cfg.sh 'udpcast_emetteur' '$corrige_mac' '$ip_machine' '$nom_machine' '$compr' '$port' '$enableDiskmodule' '$diskmodule' '$netmodule' '$disk' '$auto_reboot' '$udpcparam' '$urlse3' '$num_op'", $retour);
				...
				if(count($retour)>0){
					...
					$sql="INSERT INTO se3_tftp_action SET id='$id_emetteur',
															...
															num_op='$num_op',
															infos='compr=$compr|disk=$disk|port=$port|enableDiskmodule=$enableDiskmodule|diskmodule=$diskmodule|netmodule=$netmodule|auto_reboot=$auto_reboot|udpcparam=$udpcparam';";
